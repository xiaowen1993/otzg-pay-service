...
public class RedisService {
    ...
    // 这个是关键↓↓↓
    @Autowired
    private RedisTemplate redisTemplate; //操作0数据库
    ...
    public void setRedisJsonTemplate(RedisTemplate<String, Object> redisJsonTemplate) {
     ...
     * @return
     */
/*    public RedisTemplate switchdb(Integer db){
    ...
    public Set getByKeys(String keys) {
        if(keys==null){
            return null;
        }else {
            //redisTemplate=switchdb(db);
    ...
    public boolean hMDel(String key) {
        try {
     ...
     * 枷锁
     * @param key
     * @param timestamp 失效时间秒
     * @return
     */
    public boolean lock(String key,long timestamp){
        while (((System.currentTimeMillis() - timestamp)/1000) < 15){
            if(setIfAbsent(key,System.currentTimeMillis(),15)){
                return true;
            }
        }
        return false;
    ...
    //=====================发布订阅模式===============================
    //发布消息
    public boolean sendQue(String channel,Object message){
        try {
            redisJsonTemplate.convertAndSend(channel,message);
            return true;
        } catch (Exception e) {
            e.printStackTrace();
            return false;
