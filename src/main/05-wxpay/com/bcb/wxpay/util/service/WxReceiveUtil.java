...
//        paramMap.put("sub_openid", payOrderDto.getSubOpenid());
    ...
    final Map<String, String> receiveAppData(String subMchId, String payOrderNo, PayOrderDto payOrderDto) {
        Map<String, String> paramMap = new HashMap<>();
        paramMap.put("sub_mch_id", subMchId);
        paramMap.put("sub_app_id", payOrderDto.getAppId());
...
//        paramMap.put("sub_openid", payOrderDto.getSubOpenid());
    ...
    final Map postPayQuery(Map<String, String> map) throws Exception {
        ...
        if (null == result.get("return_code")
            ...
            return JsonUtil.get(false, RespTips.ERROR_CODE.code, "调用接口失败");
        }
        ...
        if (null == result.get("result_code")
            ...
            return JsonUtil.get(false, result.get("err_code"), "支付结果错误");
        }
        ...
        if (result.get("trade_state") != null
            ...
            return JsonUtil.get(true, result.get("trade_state"), "收款成功",result);
        } else {
            LogUtil.saveTradeLog("收款未成功");
            return JsonUtil.get(false, result.get("trade_state"), "收款未成功");
        }
    }
    ...
    //微信支付收款业务入口
    public final synchronized Map pay(String subMchId, String payOrderNo, PayOrderDto payOrderDto) {
        Map map = getInitMap();
        try {
            ...
            if (map.get("success").equals(true)) {
                ...
                LogUtil.saveTradeLog("微信支付错误=>" + map.toString());
                return getInitMap();
            }
    ...
    public final Map query(String subMchId, String payOrderNo) {
        Map map = getInitMap();
        try {
            map = postPayQuery(payQueryData(subMchId, payOrderNo));

            if (map.get("success").equals(true)) {
                return map;
            } else {
                LogUtil.saveTradeLog("微信收款查询结果=>" + map.toString());
                return getInitMap();
            }
